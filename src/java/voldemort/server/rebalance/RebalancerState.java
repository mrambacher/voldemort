...
/**
 * Store and manipulate rebalancing state. Moved out from {@link RebalancePartitionsInfo} and {@link MetadataStore}
 * 
 */
public class RebalancerState {
    private final static String NL = System.getProperty("line.separator");
    private final Map<Integer, RebalancePartitionsInfoLiveCycle> stealInfoMap;

    /**
     * Initializes internal map structure that keeps track of donorIDs to {@link RebalancePartitionsInfoLiveCycle}.
     * <p>
     * 
     * Each association represents a rebalance action between the stealer and the donor in question.
     * 
     * The execution state of this rebalance action is track using {@link RebalancePartitionsInfoLiveCycle} 
     * which contains an instance of {@link RebalancePartitionsInfoLifeCycleStatus}. All the 
     * rebalance task will be initializaed as {@link RebalancePartitionsInfoLifeCycleStatus#NEW} 
     * from the execution point of view.
     * <p>
     * 
     * Note: If a stealerNode dies while rebalancing is taking place we have to resume this 
     * failed task after the stealerNode is brought back to operation.
     * 
     * 
     * @param stealInfoList list of {@link RebalancePartitionsInfo} used to initialized the
     *            internal map structure <code>stealInfoMap</code>
     */
    public RebalancerState(List<RebalancePartitionsInfo> stealInfoList) {
        ...
        for (RebalancePartitionsInfo rebalancePartitionsInfo : stealInfoList) {
            RebalancePartitionsInfoLiveCycle pinfoLiveCycle = new RebalancePartitionsInfoLiveCycle(rebalancePartitionsInfo, RebalancePartitionsInfoLifeCycleStatus.NEW);
    ...
    public void add(RebalancePartitionsInfo rebalancePartitionsInfo, RebalancePartitionsInfoLifeCycleStatus status) {
        stealInfoMap.put(rebalancePartitionsInfo.getDonorId(), new RebalancePartitionsInfoLiveCycle(rebalancePartitionsInfo, status));
    }
