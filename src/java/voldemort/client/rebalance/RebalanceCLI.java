...
public class RebalanceCLI {
    /**
     * For automation purposes this CLI can be called by a 3rd party program.
     * The following exit error code represent exit code that will allow
     * identify if the program finished successfully or not.
     */
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            parser.accepts("parallelism", "number of rebalances to run in parallel. (default:" + RebalanceClientConfig.MAX_PARALLEL_REBALANCING + ")")
                    .withRequiredArg()
                    .ofType(Integer.class)
                    .describedAs("parallelism");
            parser.accepts("parallel-donors", "number of parallel donors to run in parallel. (default:" + RebalanceClientConfig.MAX_PARALLEL_DONOR + ")")
                    .withRequiredArg()
                    .ofType(Integer.class)
            ...
            // // We can not recover from the error, so at least report it and continue.
